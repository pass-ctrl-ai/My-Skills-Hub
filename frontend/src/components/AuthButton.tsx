...
export function AuthButton() {
  ...
  if (user) {
    return (
      <div className="relative" ref={ref}>
        ...
        {open && (
          <div className="absolute right-0 mt-2 w-56 bg-white border border-gray-200 rounded-[var(--ps-radius-card)] shadow-lg py-2 z-50">
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 bg-white border border-gray-200 rounded-[var(--ps-radius-card)] shadow-xl p-5 z-50">
          <h3 className="text-base font-semibold text-gray-900 mb-4 text-center">
            {mode === "forgot" ? t("auth.resetPassword") : mode === "signup" ? t("auth.createAccount") : t("auth.welcome")}
          </h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full px-4 py-2.5 text-sm bg-[var(--ps-gradient-neon)] text-[#0a0e1a] font-semibold rounded-lg hover:shadow-[var(--ps-shadow-btn)] transition-colors font-medium disabled:opacity-60 flex items-center justify-center gap-2 cursor-pointer"
            >
              {submitting && (
                <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
              )}
              {mode === "forgot" ? t("auth.sendReset") : mode === "signup" ? t("auth.createAccount") : t("auth.signIn")}
            </button>
          </form>
            ...
            {mode === "signin" && (
              <>
                <button
                  ...
                  className="block w-full text-xs text-gray-400 hover:text-[var(--ps-neon-cyan)] transition-colors cursor-pointer"
                >
                  {t("auth.forgotPassword")}
                </button>
                <button
                  onClick={() => switchMode("signup")}
                  className="block w-full text-xs text-[var(--ps-neon-cyan)] hover:text-blue-700 transition-colors cursor-pointer"
                >
                  {t("auth.noAccount")}
                </button>
              </>
