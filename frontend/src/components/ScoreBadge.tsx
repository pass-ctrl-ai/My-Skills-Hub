...
function getTierColor(score: number): string {
  ...
  if (score >= 65) return "text-[var(--ps-neon-cyan)]";
...
export function ScoreBadge({ score, size = "md", showTier = false }: Props) {
  ...
  if (showTier) {
    ...
    return (
      <div className={`${w} ${h} ${color} border rounded-[var(--ps-radius-card)] flex flex-col items-center justify-center shrink-0`}>
  ...
  return (
    <div
      className={`${sizeClass} ${color} border rounded-[var(--ps-radius-card)] flex items-center justify-center font-bold shrink-0`}
    >
      {score.toFixed(0)}
    </div>
  );
