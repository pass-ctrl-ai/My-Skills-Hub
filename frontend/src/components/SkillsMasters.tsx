        ...
        <>
          <div className="mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-[var(--ps-text-primary)]">{t("masters.title")}</h2>
              <span className="text-sm text-[var(--ps-text-muted)]">{t("masters.subtitle")}</span>
            </div>
            <p className="text-xs text-[var(--ps-text-muted)] mt-1.5 ml-7">
              {t("masters.warning")}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            {curated.map((m) => (
              <div
                key={m.github}
                className="bg-gradient-to-br from-white to-indigo-50/30 dark:from-gray-800/80 dark:to-indigo-900/10 border border-indigo-100 dark:border-indigo-800 rounded-[var(--ps-radius-card)] p-5 hover:shadow-lg transition-all"
              >
                {/* Header */}
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <h3 className="font-bold text-[var(--ps-text-primary)]">{m.name}</h3>
                    ...
                    <a
                      ...
                      className="text-xs text-[var(--ps-text-secondary)] hover:text-indigo-600"
                    >
                      @{m.github}
                    </a>
                    {m.x_handle && (
                      <span className="text-xs text-[var(--ps-text-muted)] ml-2">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="hover:text-[var(--ps-neon-cyan)]"
                        >
                          @{m.x_handle}
                        </a>
                        {m.x_followers > 0 && (
                          <span className="ml-1 text-[var(--ps-text-muted)]">
                    ...
                    <p className="text-xs text-[var(--ps-text-secondary)] mt-1 line-clamp-2">{m.bio}</p>
                  </div>
                </div>

                {/* Stats */}
                <div className="flex items-center gap-4 mb-3 text-sm">
                  <span className="flex items-center gap-1 text-[var(--ps-text-secondary)]">
                    <svg className="w-4 h-4 text-[var(--ps-text-muted)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {m.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-3">
                    {m.tags.map((tag) => (
                      <span key={tag} className="px-2 py-0.5 text-[10px] rounded-full bg-indigo-50 dark:bg-indigo-900/30 text-indigo-500 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div className="space-y-1.5">
                  {m.top_repos.slice(0, 4).map((r) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[var(--ps-bg-card)] border border-[var(--ps-border)] hover:border-[var(--ps-border-glow)] hover:bg-[rgba(0,240,255,0.06)] transition-all text-sm"
                    >
                      <span className="font-medium text-[var(--ps-text-primary)] truncate flex-1">{r.repo_name}</span>
                      <span className="flex items-center gap-0.5 text-xs text-[var(--ps-text-secondary)] shrink-0">
      ...
      {orgs.length > 0 && (
        <>
          <div className="flex items-center gap-2 mb-3 mt-6">
            ...
            <h3 className="text-sm font-semibold text-[var(--ps-text-primary)]">{t("emerging.title")}</h3>
            <span className="text-xs text-[var(--ps-text-muted)]">{t("emerging.subtitle")}</span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {orgs.map((o, i) => {
              ...
              return (
                <div
                  key={o.github}
                  className="ps-card border border-[var(--ps-border)] rounded-[var(--ps-radius-card)] p-4 hover:shadow-md hover:border-purple-200 dark:hover:border-purple-700 transition-all"
                >
                  <div className="flex items-center gap-3 mb-2">
                    <img
                      ...
                      loading="lazy"
                      width={40} height={40}
                      className="w-10 h-10 rounded-full border border-[var(--ps-border)]"
                    />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="font-semibold text-sm text-[var(--ps-text-primary)] hover:text-purple-600 transition-colors truncate block"
                        >
                          {o.github}
                        </a>
                      ...
                      <div className="flex items-center gap-3 text-xs text-[var(--ps-text-secondary)]">
                  ...
                  <div className="space-y-1">
                    {visibleRepos.slice(0, isExpanded ? 5 : 2).map((r) => (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center justify-between px-2 py-1.5 rounded-md hover:bg-[rgba(0,240,255,0.04)] transition-colors text-xs"
                      >
                        <span className="font-medium text-[var(--ps-text-primary)] truncate">{r.repo_name}</span>
                        <span className="flex items-center gap-0.5 text-[var(--ps-text-muted)] shrink-0 ml-2">
