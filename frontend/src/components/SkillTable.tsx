...
export function SkillTable({ skills, onSelect: _onSelect, onShowDetail }: Props) {
  ...
  return (
    <div className="overflow-x-auto ps-card border border-[var(--ps-border)] rounded-[var(--ps-radius-card)]">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-[var(--ps-border)] text-left text-[var(--ps-text-secondary)]">
        ...
        <tbody>
          {skills.map((skill) => (
              <tr
                ...
                className="border-b border-[var(--ps-border)] hover:bg-[rgba(0,240,255,0.04)] transition-colors cursor-pointer"
              >
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    <span
                      ...
                      className="font-medium text-[var(--ps-text-primary)] hover:text-[var(--ps-neon-cyan)] hover:underline transition-colors cursor-pointer"
                    >
                      {skill.repo_name}
                    </span>
                  ...
                  <div className="text-xs text-[var(--ps-text-muted)] mt-0.5">{skill.author_name}</div>
                </td>
                ...
                <td className="px-4 py-3 hidden lg:table-cell">
                  <div className="flex items-center gap-1">
                    <div className="w-16 h-1.5 bg-[var(--ps-bg-elevated)] rounded-full overflow-hidden">
                      <div
                    ...
                    <span className="text-xs text-[var(--ps-text-secondary)] tabular-nums w-6">{Math.round(skill.quality_score)}</span>
                    <span className="text-[9px] font-bold text-[var(--ps-text-muted)]">{getTier(skill.quality_score)}</span>
                  </div>
                </td>
                <td className="px-4 py-3 text-right tabular-nums text-[var(--ps-text-primary)]">{skill.stars.toLocaleString()}</td>
                <td className="px-4 py-3 text-right hidden sm:table-cell">
                  {skill.star_momentum >= 0.05 ? (
                    ...
                    <span className="text-[var(--ps-neon-cyan)]">
                      <svg className="w-3 h-3 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 15l7-7 7 7" />
                      </svg>
                    </span>
                  ) : (
                    <span className="text-[var(--ps-text-muted)] text-xs">-</span>
                  )}
                </td>
                <td className="px-4 py-3 text-right text-[var(--ps-text-muted)] hidden md:table-cell">
